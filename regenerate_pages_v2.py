...
document.addEventListener('DOMContentLoaded', () => {{
  ...
  if (bar && thumbsWrap) {{
    ...
    function scrollByOne(dir) {{
      var thumbs = bar.querySelectorAll('.media-thumb');
      if (!thumbs.length) return;
      var thumbWidth = thumbs[0].getBoundingClientRect().width;
      var margin = bar.clientWidth < thumbs[0].offsetWidth ? 8 : 0;
      var step = thumbWidth + margin;
