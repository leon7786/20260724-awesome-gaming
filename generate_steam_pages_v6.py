...
def extract_video_data(html):
    ...
    # 方法1: 提取gamehighlight_desktopcarousel的data-props
    match = re.search(r'data-featuretarget="gamehighlight-desktopcarousel"[^>]*data-props="({.*?})"', html, re.DOTALL)
    if match:
        try:
            props_json = match.group(1)
            # 解码HTML实体
            props_json = props_json.replace('&quot;', '"').replace('&amp;', '&').replace('&lt;', '<').replace('&gt;', '>')
            props = json.loads(props_json)
            
            if 'trailers' in props:
                for trailer in props['trailers']:
                    video_data = {
                        'type': 'video',
                        'hls': trailer.get('hlsManifest', ''),
                        'dash': trailer.get('dashManifests', []),
                        'poster': trailer.get('poster', ''),
                        'thumb': trailer.get('thumbnail', '')
                    }
                    videos.append(video_data)
        except Exception as e:
            print(f"  Error parsing video data: {e}")
    
    # 方法2: 如果方法1失败，尝试从application_config提取
    if not videos:
        match = re.search(r'data-config="({.*?})"[^>]*data-userinfo', html, re.DOTALL)
        if match:
            try:
                config_json = match.group(1)
                config_json = config_json.replace('&quot;', '"').replace('&amp;', '&')
                # 这里可能包含更多数据，但结构更复杂
                pass
            except:
                pass
...
def generate_detail_page(game):
    ...
    # 获取Steam页面HTML（包含HLS视频流）
    page_html = get_steam_page_html(appid)
    if page_html:
        time.sleep(0.3)
    
    # 提取视频数据
    videos = extract_video_data(page_html)
